...
int Try_Add_Color(	int Position_Color, 
   ...
   while (Edge!=NULL){
      ...
      if (Labels[i]!=Labels[j]){
      	...
      	*Num_Comp=*Num_Comp-1;
      	printf(" %d %d ", *Num_VBlocks , *Num_Comp);      getch(); //pp
         Labeli=Labels[i];
         Labelj=Labels[j];
...
void initial_MVCA(int Demo){
   ...
   for (p=0; p<c; p++){
    ...
    NumVBlocks_app[p]=NumVB;
      ...
      for (i=0; i<n; i++)	for (j=0; j<n; j++)	Label2_temp[p][i] = i * n + j;
            ...
            if (Label_temp[p][i]!=Label_temp[p][j]){
				...
				if(LabelB_temp[p][i]!=LabelB_temp[p][j]) {
         		...
         		NumVBlocks_app[p] = NumVBlocks_app[p] + 1;
      			...
      			Label2i = Label2_temp[p][k*n+path[k]] ;
         		k = i ;
      			do{
      				if (Label2_temp[p][k*n+path[k]] != Label2i){  
					// Decrease the number of VBlocks
						NumVBlocks_app[p] = NumVBlocks_app[p] - 1;
						printf("[%d,%d]: [%d,%d], ",i,j,k,path[k]); getch(); //pp
