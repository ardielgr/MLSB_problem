...
#define Size 21	// maximum Number of vertices and maximum Number of colors
#define Size2 500	// maximum Number of vertices and maximum Number of colors
...
Vector MVCA_Sol;   // Solution provided by MVCA

int Demo;         

long int call,Best_call,previous_call;
...
void Read_Data(){
	int i,j,k;
   for (i=0; i<n-1; i++)  // vertices da from a n-1
   	for(j=i+1; j<n; j++)
		      	Color_Matrix[i][j]  = ReadNumber(Data_File);
   if (Demo==1){
   	printf("Color_Matrix:\n");
   	for (i=0; i<n-1; i++){
   		for(j=i+1; j<n; j++)
...
void q_sortGF(int left, int right) {  
   ...
   if (Demo==1){
   	...
   	printf("\nSort List: ");
   	printf("\ncolor(frequency): ");
		for (k=0; k<c; k++) printf("%d(%d) ",Colors[k],Freq[k]);
		printf("\n\n");
   	getch();
   }
...
int Try_Add_Color(	int Position_Color, 
   ...
   for (i=0; i<n*n; i++) Labels2_support_Add[i] = Labels2[i];
   ...
   while (Edge!=NULL){
      ...
      if (Labels[i]!=Labels[j]){
      	*Num_Comp=*Num_Comp-1;           // Num of comps decrease

         Labeli=Labels[i];  // Joning Comps of vertices i and j
         Labelj=Labels[j];
         for (k=0; k<n; k++){	
         	if (Labels[k]==Labelj)
            	   Labels[k]=Labeli;
      	...
      	*Num_VBlocks = *Num_VBlocks + 1; // edge is a bridge; i.e., a new VBlocks
      	if (i>j){ Labels2[i*n+j]=i*n+j;}
			   else { Labels2[i*n+j]=j*n+i;}
			Labels2[j*n+i]=Labels2[i*n+j];  // Set label2 of edge [i,j]
   ...
   if (((*Num_VBlocks<app_Num_VBlocks)&&(*Num_Sol<Best_Num_Sol-2))
      ...
      *Num_Sol=*Num_Sol+1;
      ...
      if (Demo==1){
   		if ((*Num_VBlocks<app_Num_VBlocks)&&(*Num_Sol<Best_Num_Sol-2))	
		   		printf(" Don't add %d because it doesn't decrease the Number of V Blocks %d!\n",
      ...
      *Num_VBlocks=app_Num_VBlocks;
      ...
      for (i=0; i<n*n; i++)	Labels2[i]=Labels2_support_Add[i];
   ...
   if (((call%1000)==0)&&(previous_call!=call)){
      ...
      printf("\n VBlocks: %d. Blocks: %d. Comp: %d\n",*Num_VBlocks,*Num_Blocks,*Num_Comp);
   }
...
void Try(int Position_Color, 
   ...
   if (Num_VBlocks==1){
      ...
      if (continue_to_try==1){
        ...
        for (i=0; i<n*n; i++)	app_Labels2[i]=Labels2[i];
        	k=Position_Color+1;
        	while((k<c)&&(Num_Sol<Best_Num_Sol-1)){
            	...
            	for (i=0; i<n; i++) Labels[i]=app_Labels[i];
         	    ...
         	    for (i=0; i<n; i++) for (j=0; j<n; j++) Labels2[i*n+j]=app_Labels2[i*n+j];
         	    Try(k,	
					Labels,Num_Comp,
					LabelsB,Num_Blocks,
					Labels2,Num_VBlocks,
...
void initial_MVCA(int Demo){
   ...
   for (p=0; p<c; p++){
      ...
      for (i=0; i<n; i++)	for (j=0; j<n; j++)	
         if (i>j) { Label2_temp[p][i*n+j]=i*n+j;}
			   else { Label2_temp[p][i*n+j]=j*n+i;}
			Label2_temp[p][j*n+i] = Label2_temp[p][i*n+j];
   	...
   	for (p=0; p<c; p++) {
         ...
         while (e!=NULL) {
            ...
            if (Label_temp[p][i]!=Label_temp[p][j]){
      			...
      			NumVBlocks_app[p]=NumVBlocks_app[p]+1;
         		...
         		// The path is i, path[i], path[path[i]], ..., j
				  ...
				  // if(LabelB_temp[p][i]!=LabelB_temp[p][j]) { ONLY FOR E-BLOCKS
					
					
         		// Traverse the path from i to j joining the blocks found
         		NumVBlocks_app[p] = NumVBlocks_app[p] + 1;
      			
					Label2i = Label2_temp[p][ i*n + j] ;				
         		
					Labeli = LabelB_temp[p][i];

      			k = i ;
      			do{      				
     				if (Label2_temp[p][k*n+path[k]] != Label2i){  
					// Decrease the number of VBlocks
						NumVBlocks_app[p] = NumVBlocks_app[p] - 1;
      				Label2k = Label2_temp[p][k*n+path[k]];
				  		for(int q=0; q<n; q++)
				  			for(int qq=0; qq<n; qq++)
				  			if (Label2_temp[p][q*n+qq]==Label2k){
				  				Label2_temp[p][q*n+qq]= Label2i ;
				  				Label2_temp[p][qq*n+q]= Label2i ;
							  }
      					}	
				...
				Label2_temp[p][i*n+j]=Label2k;
				Label2_temp[p][j*n+i]=Label2k;
				} // END OF ELSE
...
*/            
      ...
      a=new Adj_item;
         ...
         if (Demo==1){
         	printf("\n\n -----> NumVBlocks_app[%d]=%d, NumBlocks_app[%d]=%d, NumComps_app[%d]=%d\n\n",
			 		p,NumVBlocks_app[p],p,NumBlocks_app[p],p,NumComps_app[p]);
         	getch();
         	printf("\n OLD TEMP Num=%d\n",NumT);
         	getch();
         }
        	if (NumVBlocks_app[p]+NumBlocks_app[p]+NumComps_app[p]<NumT) {
				Add_p=p;
            NumC=NumComps_app[p];
				NumB=NumBlocks_app[p];
				NumVB=NumVBlocks_app[p];
				NumT=NumC+NumB+NumVB;
         }
         if (Demo==1){
         	printf("\nTEMPORARY  Add_p=%d, Num=%d\n",Add_p,NumT);
      ...
      if (Demo==1)	printf("Adding color %d: \n",Colors[Add_p]);
      ...
      for (j=0; j<c; j++){
         ...
         if (temp[j]==1)	continue;
      	for (i=0; i<n; i++) {
			...
			for (i = 0; i < n*n; i++)
         	Label2_temp[j][i]=Label2_temp[Add_p][i];
      }
    }
    if(Demo==1){
