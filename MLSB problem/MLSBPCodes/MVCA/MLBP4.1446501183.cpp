...
#define Size 101	// maximum Number of vertices and maximum Number of colors
#define Size2 10201	// maximum Number of vertices and maximum Number of colors
...
void Print_Summary(	
   ...
   printf("\n Labels2: "); for (i=0; i<n; i++){
   							printf("\n     %d: ",i);  
							for(j=0; j<n; j++) printf("%d ",Labels2[i*n+j]);
      						}	
    					    printf("\n");
            ...
            if (Label_temp[p][i]!=Label_temp[p][j]){
				...
				if(LabelB_temp[p][i]!=LabelB_temp[p][j]) {
         		...
         		NumVBlocks_app[p] = NumVBlocks_app[p] + 1;
					...
					Label2i = Label2_temp[p][ i*n + path[i]] ;				
